...
public class AreaSelection
{
    ...
    public boolean renameSubRegionBox(String oldName, String newName, @Nullable IMessageConsumer feedback)
    {
        ...
        if (box != null)
        {
            if (this.subRegionBoxes.containsKey(newName))
            {
                if (feedback != null)
                {
                    feedback.addMessage(MessageType.ERROR, "simplematica.error.area_editor.rename_sub_region.exists", newName);
                }
    ...
    public void moveEntireSelectionTo(BlockPos newOrigin, boolean printMessage)
    {
        ...
        if (printMessage)
        {
            ...
            InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS, "simplematica.message.moved_selection", oldStr, newStr);
        }
    }
