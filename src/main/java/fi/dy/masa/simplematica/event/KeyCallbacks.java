...
import fi.dy.masa.simplematica.gui.*;
...
import fi.dy.masa.simplematica.selection.CornerSelectionMode;
...
//        Configs.Visuals.ENABLE_SCHEMATIC_ENTITY_HITBOXES.setValueChangeCallback(renderChangeCallback); // Not needed
    ...
    private record KeyCallbackHotkeys(Minecraft mc) implements IHotkeyCallback
    {
        ...
        public boolean onKeyAction(KeyAction action, IKeybind key)
        {
            ...
            {
                SchematicWorldRefresher.INSTANCE.updateAll();
                InfoUtils.printActionbarMessage("simplematica.message.schematic_rendering_refreshed");
                return true;
            }
                ...
                if (mode.getUsesAreaSelection())
                {
                    ...
                    if (selection != null)
                    {
                        if (selection.isOriginSelected())
                        {
                            ...
                            InfoUtils.printActionbarMessage("simplematica.message.removed_area_origin");
                        }
                        else
                        {
                            ...
                            {
                                InfoUtils.printActionbarMessage("simplematica.message.removed_selection_box", name);
                                return true;
                            }
                        }
                    }
            ...
            else if (key == Hotkeys.SCHEMATIC_PLACEMENT_ROTATION.getKeybind())
            {
                ...
                if (placement != null)
                {
                    ...
                    else
                    {
                        ...
                        InfoUtils.printActionbarMessage("simplematica.message.placement.rotation_set_to", PositionUtils.getRotationNameShort(rotation));
                    }
                    return true;
                }
            }
            else if (key == Hotkeys.SCHEMATIC_PLACEMENT_MIRROR.getKeybind())
            {
                ...
                if (placement != null)
                {
                    ...
                    else
                    {
                        ...
                        InfoUtils.printActionbarMessage("simplematica.message.placement.mirror_set_to", PositionUtils.getMirrorName(mirror));
                    }
                    return true;
                }
            }
            ...
            else if (key == Hotkeys.SET_AREA_ORIGIN.getKeybind())
            {
                if (mode.getUsesAreaSelection() && this.mc.player != null)
                {
                    ...
                    if (area != null)
                    {
                        ...
                        InfoUtils.printActionbarMessage("simplematica.message.set_area_origin", posStr);
                        return true;
                    }
                }
            }
                     ...
                     key == Hotkeys.SET_SELECTION_BOX_POSITION_2.getKeybind())
            {
                if (mode.getUsesAreaSelection() && this.mc.player != null)
                {
                    ...
                    if (area != null && area.getSelectedSubRegionBox() != null)
                    {
                        ...
                        String posStr = String.format("x: %d, y: %d, z: %d", pos.getX(), pos.getY(), pos.getZ());
                        InfoUtils.printActionbarMessage("simplematica.message.set_selection_box_point", corner.ordinal(), posStr);
                        return true;
                    }
                }
            }
