...
import fi.dy.masa.simplematica.scheduler.ITask;
import fi.dy.masa.simplematica.scheduler.TaskTimer;
...
public abstract class TaskBase implements ITask, IInfoHudRenderer
{
    ...
    protected void updateInfoHudLinesPendingChunks(Collection<ChunkPos> pendingChunks)
    {
        ...
        if (pendingChunks.isEmpty() == false && this.mc.player != null)
        {
            ...
            String title = StringUtils.translate("simplematica.gui.label.task.title.remaining_chunks", this.getDisplayName(), pendingChunks.size());
