...
public class ChunkRenderGpuDispatcher
{
	...
	protected ChunkRenderGpuUploader addOrGetUploader(final int cx, final int cz)
	{
		...
		catch (Exception e)
		{
			...
			{
				Simplematica.debugLog("addOrGetUploader: Exception obtaining a Chunk Uploader; {}", e.getLocalizedMessage());
			}
		}
	...
	protected void removeUploader(final int cx, final int cz)
	{
		...
		catch (Exception e)
		{
			if (Reference.DEBUG_MODE)
			{
				Simplematica.debugLog("removeUploader: Exception removing a Chunk Uploader; {}", e.getLocalizedMessage());
			}
		}
	}
