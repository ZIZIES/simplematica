package fi.dy.masa.simplematica.render;
...
import fi.dy.masa.simplematica.compat.jade.JadeCompat;
...
import fi.dy.masa.simplematica.gui.widgets.WidgetSchematicVerificationResult.BlockMismatchInfo;
...
import fi.dy.masa.simplematica.schematic.verifier.SchematicVerifier.MismatchRenderPos;
...
import fi.dy.masa.simplematica.util.BlockInfoAlignment;
import fi.dy.masa.simplematica.util.InventoryUtils;
...
public class OverlayRenderer
{
    ...
    private void renderSchematicMismatches(List<MismatchRenderPos> posList, @Nullable BlockPos lookPos, ProfilerFiller profiler)
    {
        ...
        RenderContext ctx = new RenderContext(() -> "simplematica:schematic_mistaches/batched_lines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_NO_DEPTH_NO_CULL, 0);
        ...
        if (lookedEntry != null)
        {
	        ...
	        lineWidth = 6f;
            buffer = ctx.start(() -> "simplematica:schematic_mistaches/outlines", MaLiLibPipelines.DEBUG_LINES_MASA_SIMPLE_NO_DEPTH_NO_CULL, 0);
        ...
        {
            buffer = ctx.start(() -> "simplematica:schematic_mistaches/side_quads", MaLiLibPipelines.POSITION_COLOR_TRANSLUCENT_NO_DEPTH_NO_CULL, 0);
    ...
    private void renderBlockInfoOverlay(GuiContext ctx, RayTraceWrapper traceWrapper)
    {
        ...
        {
            BlockInfo info = new BlockInfo(stateClient, "simplematica.gui.label.block_info.state_client");
            this.getOverlayPosition(align, info.getTotalWidth(), info.getTotalHeight(), offY, invHeight);
            info.toggleUseBackgroundMask(true);
            info.render(ctx, this.blockInfoX, this.blockInfoY);
        }
        ...
        {
            BlockInfo info = new BlockInfo(stateSchematic, "simplematica.gui.label.block_info.state_schematic");
            this.getOverlayPosition(align, info.getTotalWidth(), info.getTotalHeight(), offY, invHeight);
            info.toggleUseBackgroundMask(true);
            info.render(ctx, this.blockInfoX, this.blockInfoY);
        }
