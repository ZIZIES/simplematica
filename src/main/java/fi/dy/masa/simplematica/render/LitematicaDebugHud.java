...
import fi.dy.masa.simplematica.util.DebugHudMode;
...
public class LitematicaDebugHud implements DebugScreenEntry
{
	...
	public List<String> getDebugLines()
	{
		...
		if (worldSchematic != null)
		{
			...
			if (this.isLeft())
			{
				list.add(String.format("%s[Simplematica]%s %s",
				                       pre, rst,
				...
				{
					list.add(String.format("%s[Simplematica]%s %s %s",
					                       pre, rst,
			...
			{
				list.add(String.format("%s %s[Simplematica]%s",
				                       renderer.getDebugInfoRenders(),
				                       rst+pre, rst
				));
				...
				if (Configs.Generic.DEBUG_HUD_WORLD.getBooleanValue())
				{
					list.add(String.format("%s %s %s[Simplematica]%s",
					                       renderer.getDebugInfoEntities(),
					                       str,
					                       rst + pre, rst
					));
		...
		else
		{
			...
			if (this.isLeft())
			{
				list.add(String.format("%s[Simplematica]%s %s",
				                       pre, rst+red,
				                       StringUtils.translate("simplematica.gui.message.debug_hud.not_loaded")
				));
			}
			else
			{
