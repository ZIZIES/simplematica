...
//        RenderSystem.assertOnRenderThread();
        ...
        if (!this.chunkRenderers.isEmpty())
        {
                                   ...
                                   .removeIf(entry ->
                                    {
                                        ...
                                        if (cr != null && (cr.getDistanceSq() > this.viewDistanceBlocksSq || cr.isEmpty()))
                                        {
                                            ...
                                            catch (Exception e)
                                            {
                                                ...
                                                {
                                                    Simplematica.debugLog("removeOutOfRangeRenderers: cr.close() threw an exception; {}", e.getLocalizedMessage());
                                                }
                                            }
            ...
            catch (Exception e)
            {
                if (Reference.DEBUG_MODE)
                {
                    Simplematica.debugLog("removeOutOfRangeRenderers: keySet() threw an exception; {}", e.getLocalizedMessage());
                }
            }

            if (Reference.DEBUG_MODE && prevCount != this.chunkRenderers.size())
            {
                Simplematica.LOGGER.warn("[Dispatch] removeOutOfRangeRenderers: [{}] -> [{}]", prevCount, this.chunkRenderers.size());
            }
        }
    }
...
//        this.getOrCreateChunkRenderer(chunkX, chunkZ).ifPresent(cr -> cr.setNeedsUpdate(immediate));
    ...
    protected Optional<ChunkRendererSchematicVbo> getOrCreateChunkRenderer(int chunkX, int chunkZ)
    {
        ...
        try
        {
            ...
            {
//                Simplematica.LOGGER.warn("[Dispatch] chunkRenderer[{}, {}] does not exist, factory create -->", chunkX, chunkZ);
        ...
        catch (Exception e)
        {
            if (Reference.DEBUG_MODE)
            {
                Simplematica.debugLog("getOrCreateChunkRenderer: Exception obtaining a Chunk Renderer; {}", e.getLocalizedMessage());
            }
        }
