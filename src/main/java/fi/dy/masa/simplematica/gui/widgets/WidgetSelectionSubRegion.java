...
import fi.dy.masa.simplematica.gui.GuiAreaSelectionEditorSubRegion;
...
public class WidgetSelectionSubRegion extends WidgetListEntryBase<String>
{
    ...
    public void postRenderHovered(GuiContext ctx, int mouseX, int mouseY, boolean selected)
    {
        ...
        if (this.box != null)
        {
            ...
            if (pos1 != null)
            {
                String str = StringUtils.translate("simplematica.gui.label.area_editor.pos1");
                text.add(String.format("%s: x: %d, y: %d, z: %d", str, pos1.getX(), pos1.getY(), pos1.getZ()));
            }

            if (pos2 != null)
            {
                String str = StringUtils.translate("simplematica.gui.label.area_editor.pos2");
                text.add(String.format("%s: x: %d, y: %d, z: %d", str, pos2.getX(), pos2.getY(), pos2.getZ()));
            }

            if (pos1 != null && pos2 != null)
            {
                String str = StringUtils.translate("simplematica.gui.label.area_editor.dimensions");
			...
			implements IButtonActionListener
	{
		...
		public void actionPerformedWithButton(ButtonBase button, int mouseButton)
		{
			...
			{
				String title = "simplematica.gui.title.rename_area_sub_region";
