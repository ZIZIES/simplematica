...
import fi.dy.masa.simplematica.gui.GuiPlacementConfiguration;
...
public class WidgetSchematicPlacement extends WidgetListEntryBase<SchematicPlacement>
{
    ...
    public void postRenderHovered(GuiContext ctx, int mouseX, int mouseY, boolean selected)
    {
        ...
        {
            String str = StringUtils.translate("simplematica.hud.schematic_placement.hover_info.placement_locked");
        ...
        {
            String str = StringUtils.translate("simplematica.hud.schematic_placement.hover_info.placement_modified");
            RenderUtils.drawHoverText(ctx, mouseX, mouseY, ImmutableList.of(str));
        }
        else if (GuiBase.isMouseOver(mouseX, mouseY, this.x, this.y, this.buttonsStartX - 18, this.height))
        {
            ...
            String fileName = schematicFile != null ? schematicFile.getFileName().toString() : StringUtils.translate("simplematica.gui.label.schematic_placement.in_memory");
            List<String> text = new ArrayList<>();
            text.add(StringUtils.translate("simplematica.gui.label.schematic_placement.schematic_name", this.placement.getSchematic().getMetadata().getName()));
            text.add(StringUtils.translate("simplematica.gui.label.schematic_placement.schematic_file", fileName));
            BlockPos o = this.placement.getOrigin();
            String strOrigin = String.format("x: %d, y: %d, z: %d", o.getX(), o.getY(), o.getZ());
            text.add(StringUtils.translate("simplematica.gui.label.schematic_placement.origin", strOrigin));
            text.add(StringUtils.translate("simplematica.gui.label.schematic_placement.sub_region_count", String.valueOf(this.placement.getSubRegionCount())));

            Vec3i size = this.placement.getSchematic().getTotalSize();
            String strSize = String.format("%d x %d x %d", size.getX(), size.getY(), size.getZ());
            text.add(StringUtils.translate("simplematica.gui.label.schematic_placement.enclosing_size", strSize));
	...
	public record ButtonListener(ButtonType type, WidgetSchematicPlacement widget) implements IButtonActionListener
	{
			...
			else if (this.type == ButtonType.REMOVE)
			{
				if (this.widget.placement.isLocked() && GuiBase.isShiftDown() == false)
				{
					this.widget.parent.getParentGui()
									  .addMessage(MessageType.ERROR, "simplematica.error.schematic_placements.remove_fail_locked");
				}
				else
				{
		...
		{
			CONFIGURE       ("simplematica.gui.button.schematic_placements.configure"),
			REMOVE          ("simplematica.gui.button.schematic_placements.remove"),
