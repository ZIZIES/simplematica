package fi.dy.masa.simplematica.gui.widgets;
...
import fi.dy.masa.simplematica.gui.GuiSchematicBrowserBase;
import fi.dy.masa.simplematica.gui.Icons;
import fi.dy.masa.simplematica.materials.MaterialListCustom;
import fi.dy.masa.simplematica.materials.MaterialListPreview;
import fi.dy.masa.simplematica.schematic.LitematicaSchematic;
import fi.dy.masa.simplematica.schematic.SchematicMetadata;
import fi.dy.masa.simplematica.schematic.SchematicSchema;
...
public class WidgetSchematicBrowser extends WidgetFileBrowserBase
{
    ...
    public WidgetSchematicBrowser(int x, int y, int width, int height, GuiSchematicBrowserBase parent, @Nullable ISelectionListener<DirectoryEntry> selectionListener)
    {
        ...
        this.title = StringUtils.translate("simplematica.gui.title.schematic_browser");
        this.infoWidth = 170;
        this.infoHeight = 310;
        this.parent = parent;
    }
    ...
    protected void drawSelectedSchematicInfo(GuiContext ctx, @Nullable DirectoryEntry entry)
    {
        ...
        if (meta != null)
        {
            ...
            String str = StringUtils.translate("simplematica.gui.label.schematic_info.name");
            ...
            str = StringUtils.translate("simplematica.gui.label.schematic_info.schematic_author", meta.getAuthor());
            this.drawString(ctx, str, x, y, textColor);
            y += 12;

            String strDate = DATE_FORMAT.format(new Date(meta.getTimeCreated()));
            str = StringUtils.translate("simplematica.gui.label.schematic_info.time_created", strDate);
            this.drawString(ctx, str, x, y, textColor);
            y += 12;
            ...
            if (meta.hasBeenModified())
            {
                ...
                str = StringUtils.translate("simplematica.gui.label.schematic_info.time_modified", strDate);
                this.drawString(ctx, str, x, y, textColor);
                y += 12;
            }

            str = StringUtils.translate("simplematica.gui.label.schematic_info.region_count", meta.getRegionCount());
            this.drawString(ctx, str, x, y, textColor);
            y += 12;

            if (this.parent.getScreenHeight() >= 340)
            {
                str = StringUtils.translate("simplematica.gui.label.schematic_info.total_volume", meta.getTotalVolume());
                this.drawString(ctx, str, x, y, textColor);
                y += 12;

                if (meta.getTotalBlocks() > 0)
                {
                    str = StringUtils.translate("simplematica.gui.label.schematic_info.total_blocks", meta.getTotalBlocks());
                    this.drawString(ctx, str, x, y, textColor);
                    y += 12;
                }

                str = StringUtils.translate("simplematica.gui.label.schematic_info.enclosing_size");
                this.drawString(ctx, str, x, y, textColor);
                y += 12;
            ...
            else
            {
                if (meta.getTotalBlocks() > 0)
                {
                    str = StringUtils.translate("simplematica.gui.label.schematic_info.total_blocks_and_volume", meta.getTotalBlocks(), meta.getTotalVolume());
                    this.drawString(ctx, str, x, y, textColor);
                    y += 12;
                }
                else
                ...
                Vec3i areaSize = meta.getEnclosingSize();
                String tmp = String.format("%d x %d x %d", areaSize.getX(), areaSize.getY(), areaSize.getZ());
                str = StringUtils.translate("simplematica.gui.label.schematic_info.enclosing_size_value", tmp);
                this.drawString(ctx, str, x, y, textColor);
                y += 12;
            }
            ...
            if (version != null)
            {
                switch (meta.getFileType())
                {
                    ...
                    {
                        str = StringUtils.translate("simplematica.gui.label.schematic_info.version", version.litematicVersion());
                        this.drawString(ctx, str, x, y, textColor);
                        y += 12;
                    }
                    case SPONGE_SCHEMATIC ->
                    {
                        str = StringUtils.translate("simplematica.gui.label.schematic_info.sponge_version", version.litematicVersion());
                        this.drawString(ctx, str, x, y, textColor);
                        y += 12;
                    }
                    case VANILLA_STRUCTURE ->
                    {
                        str = StringUtils.translate("simplematica.gui.label.schematic_info.vanilla_version");
                        this.drawString(ctx, str, x, y, textColor);
                        y += 12;
                    }
                    // Not supported
//                    case SCHEMATICA_SCHEMATIC ->  {}
                ...
                if (schema != null)
                {
                    ...
                    {
                        str = StringUtils.translate("simplematica.gui.label.schematic_info.schema.newer", schema.getString(), version.minecraftDataVersion());
                    }
                    else
                    {
                        str = StringUtils.translate("simplematica.gui.label.schematic_info.schema", schema.getString(), version.minecraftDataVersion());
                    }
                    this.drawString(ctx, str, x, y, textColor);
                    y += 12;
                }
            ...
            /*
            str = StringUtils.translate("simplematica.gui.label.schematic_info.description");
        ...
        else        // Material List Info Panel
        {
            ...
            String str = StringUtils.translate("simplematica.gui.label.material_list_info.title_colon");
            this.drawString(ctx, str, x, y, textColor);
            y += 12;
            ...
            str = StringUtils.translate("simplematica.gui.label.material_list_info.name");
            this.drawString(ctx, str, x, y, textColor);
            y += 12;
            ...
            str = StringUtils.translate("simplematica.gui.label.material_list_info.item_count");
            this.drawString(ctx, str, x, y, textColor);
            y += 12;
    ...
    private void createPreviewImage(Path file, SchematicMetadata meta)
    {
        ...
        if (previewImageData != null && previewImageData.length > 0)
        {
            ...
            if ((size * size) == previewImageData.length)
            {
                ...
                {
                    Simplematica.LOGGER.warn("Failed to create a preview image", e);
                }
            }
        }
    }
