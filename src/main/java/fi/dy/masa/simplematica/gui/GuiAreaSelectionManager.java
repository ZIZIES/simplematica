...
import fi.dy.masa.simplematica.gui.widgets.WidgetAreaSelectionBrowser;
...
public class GuiAreaSelectionManager extends GuiListBase<DirectoryEntry, WidgetDirectoryEntry, WidgetAreaSelectionBrowser> implements ISelectionListener<DirectoryEntry>
{
    ...
    public GuiAreaSelectionManager()
    {
        ...
        this.title = StringUtils.translate("simplematica.gui.title.area_selection_manager");
    ...
    public void initGui()
    {
        ...
        {
            InfoUtils.showGuiMessage(MessageType.WARNING, "simplematica.message.warn.area_selection.browser_open_in_simple_mode");
        }
    }
    ...
    protected void reCreateGuiElements()
    {
        ...
        if (currentSelection != null)
        {
            ...
            if (currentSelection.length() > len + 1)
            {
                ...
                String str = StringUtils.translate("simplematica.gui.label.area_selection_manager.current_selection", currentSelection);
    ...
    private int createButton(int x, int y, ButtonListener.ButtonType type)
    {
        ...
        {
            button.setHoverStrings("simplematica.gui.button.hover.area_selections.unselect");
        }
	...
	private record ButtonListener(ButtonType type, GuiAreaSelectionManager gui) implements IButtonActionListener
	{
		...
		public void actionPerformedWithButton(ButtonBase button, int mouseButton)
		{
			if (this.type == ButtonType.NEW_SELECTION)
			{
				...
				String title = "simplematica.gui.title.create_area_selection";
			...
			else if (this.type == ButtonType.FROM_PLACEMENT)
			{
				...
				{
					Path dir = this.gui.getListWidget().getCurrentDirectory();
					String title = "simplematica.gui.title.create_area_selection_from_placement";
				...
				{
					this.gui.addMessage(MessageType.ERROR, "simplematica.error.area_selection.no_placement_selected");
				}
			}
		...
		{
			NEW_SELECTION("simplematica.gui.button.area_selections.create_new_selection"),
			FROM_PLACEMENT("simplematica.gui.button.area_selections.create_selection_from_placement"),
			UNSELECT("simplematica.gui.button.area_selections.unselect");
			...
			implements IStringConsumerFeedback
	{
		...
		public boolean setString(String name)
		{
			...
			{
				this.gui.addMessage(MessageType.SUCCESS, "simplematica.message.area_selections.selection_created_from_placement", name);
				this.gui.getListWidget().refreshEntries();
				return true;
			}
