...
import fi.dy.masa.simplematica.gui.GuiSchematicManager.ExportType;
...
public class GuiSchematicSaveExported extends GuiSchematicSaveBase
{
    ...
    public GuiSchematicSaveExported(DirectoryEntryType type, Path dirSource, String inputFileName, ExportType exportType)
    {
        ...
        this.title = StringUtils.translate("simplematica.gui.title.save_exported_schematic", exportType.getDisplayName(), inputFileName);
            ...
            implements IButtonActionListener
    {
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton)
        {
            if (this.type == ButtonType.SAVE)
            {
                ...
                if (this.gui.type == DirectoryEntryType.FILE)
                {
                    ...
                    if (fileType == FileType.LITEMATICA_SCHEMATIC)
                    {
                        if (this.gui.exportType == ExportType.V6_LITEMATIC)
                        {
                            ...
                            {
                                this.gui.addMessage(MessageType.SUCCESS, "simplematica.message.litematic_downgrade_exported_as", fileName);
                        ...
                        else if (this.gui.exportType == ExportType.SCHEMATIC)
                        {
                            ...
                            {
                                this.gui.addMessage(MessageType.SUCCESS, "simplematica.message.schematic_exported_as", fileName);
                                this.gui.getListWidget().refreshEntries();
                            }
                        }
                ...
                this.gui.addMessage(MessageType.ERROR, "simplematica.error.schematic_export.unsupported_type", this.gui.inputFileName);
            }
        }
    }
}
