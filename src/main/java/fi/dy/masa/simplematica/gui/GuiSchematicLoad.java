...
import fi.dy.masa.simplematica.materials.MaterialListSchematic;
...
public class GuiSchematicLoad extends GuiSchematicBrowserBase implements ISelectionListener<DirectoryEntry>
{
    public GuiSchematicLoad()
    {
        ...
        this.title = StringUtils.translate("simplematica.gui.title.load_schematic");
    }
	...
	private void createButtons()
	{
		...
		label = StringUtils.translate("simplematica.gui.label.schematic_load.checkbox.create_placement");
		String hover = StringUtils.translate("simplematica.gui.label.schematic_load.hoverinfo.create_placement");
    ...
    private int createButton(int x, int y, int width, ButtonListener.Type type)
    {
        ...
        {
            button.setHoverStrings(StringUtils.translate("simplematica.gui.button.hover.material_list_shift_to_select_sub_regions"));
        }
	...
	private record ButtonListener(Type type, GuiSchematicLoad gui) implements IButtonActionListener
	{
		...
		public void actionPerformedWithButton(ButtonBase button, int mouseButton)
		{
			...
			else
			{
				...
				{
					this.gui.addMessage(MessageType.ERROR, "simplematica.error.schematic_load.unsupported_type", file.getFileName());
				}
				...
				if (schematic != null)
				{
					if (this.type == Type.LOAD_SCHEMATIC)
					{
						...
						this.gui.addMessage(MessageType.SUCCESS, "simplematica.info.schematic_load.schematic_loaded", file.getFileName());
					...
					else if (this.type == Type.MATERIAL_LIST)
					{
						if (GuiBase.isShiftDown())
						{
							...
							GuiStringListSelection gui = new GuiStringListSelection(schematic.getAreas()
							                                                                 .keySet(), creator);
							gui.setTitle(StringUtils.translate("simplematica.gui.title.material_list.select_schematic_regions", schematic.getMetadata()
							                                                                                                           .getName()));
					...
					if (warnType)
					{
						InfoUtils.showGuiOrInGameMessage(MessageType.WARNING, 15000, "simplematica.message.warn.schematic_load_non_litematica");
					}
				}
			}
		}
				...
				implements IStringConsumerFeedback
		{
			...
			public boolean setString(String string)
			{
				...
				{
					this.gui.setString(StringUtils.translate("simplematica.error.schematic_rename.read_failed"));
				}
		...
		public enum Type
		{
			LOAD_SCHEMATIC      ("simplematica.gui.button.load_schematic_to_memory"),
			MATERIAL_LIST       ("simplematica.gui.button.material_list"),
			RENAME_SCHEMATIC    ("simplematica.gui.button.rename_schematic"),
			RENAME_FILE         ("simplematica.gui.button.rename_file"),
			;
