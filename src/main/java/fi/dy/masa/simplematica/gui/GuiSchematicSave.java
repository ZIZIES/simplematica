...
public class GuiSchematicSave extends GuiSchematicSaveBase implements ICompletionListener
{
    ...
    public GuiSchematicSave(@Nullable LitematicaSchematic schematic)
    {
        ...
        if (schematic != null)
        {
            this.title = StringUtils.translate("simplematica.gui.title.save_schematic_from_memory");
        }
        else
        {
            this.title = StringUtils.translate("simplematica.gui.title.create_schematic_from_selection");
        }
            ...
            implements IButtonActionListener
    {
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton)
        {
            ...
            if (this.type == ButtonType.SAVE)
            {
                ...
                if (fileName.isEmpty())
                {
                    this.gui.addMessage(MessageType.ERROR, "simplematica.error.schematic_save.invalid_schematic_name", fileName);
                    return;
                }
                ...
                if (this.gui.schematic != null)
                {
                    ...
                    if (schematic.writeToFile(dir, fileName, GuiBase.isShiftDown()))
                    {
                        ...
                        this.gui.addMessage(MessageType.SUCCESS, "simplematica.message.schematic_saved_as", fileName);
                    }
                }
                else
                {
                    ...
                    if (area != null)
                    {
                        ...
                        {
                            this.gui.addMessage(MessageType.ERROR, "simplematica.error.schematic_write_to_file_failed.exists", fileNameTmp);
                            return;
                        }
                        ...
                        this.gui.addMessage(MessageType.INFO, "simplematica.message.schematic_save_task_created");
                    }
                    else
                    {
                        this.gui.addMessage(MessageType.ERROR, "simplematica.message.error.schematic_save_no_area_selected");
                    }
                }
            }
        }
