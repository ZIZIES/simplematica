...
import fi.dy.masa.simplematica.gui.GuiSchematicVerifier.BlockMismatchEntry;
import fi.dy.masa.simplematica.gui.widgets.WidgetListSchematicVerificationResults;
import fi.dy.masa.simplematica.gui.widgets.WidgetSchematicVerificationResult;
import fi.dy.masa.simplematica.render.infohud.InfoHud;
import fi.dy.masa.simplematica.render.infohud.RenderPhase;
import fi.dy.masa.simplematica.schematic.placement.SchematicPlacement;
import fi.dy.masa.simplematica.schematic.verifier.SchematicVerifier;
import fi.dy.masa.simplematica.schematic.verifier.SchematicVerifier.BlockMismatch;
import fi.dy.masa.simplematica.schematic.verifier.SchematicVerifier.MismatchType;
                                    ...
                                    implements ISelectionListener<BlockMismatchEntry>, ICompletionListener
{
    ...
    public GuiSchematicVerifier(SchematicPlacement placement)
    {
        ...
        this.title = StringUtils.translate("simplematica.gui.title.schematic_verifier", placement.getName());
    ...
    public void initGui()
    {
        ...
        {
            String str = StringUtils.translate("simplematica.gui.label.schematic_verifier.status.verifying", this.verifier.getUnseenChunks(), this.verifier.getTotalChunks());
        ...
        else if (this.verifier.isFinished())
        {
            ...
            String str = StringUtils.translate("simplematica.gui.label.schematic_verifier.status.done_errors.no_diff", wb, ws, m, e);

            if (Configs.Generic.ENABLE_DIFFERENT_BLOCKS.getBooleanValue())
            {
                str = StringUtils.translate("simplematica.gui.label.schematic_verifier.status.done_errors", wb, ws, m, e, d);
            }

            this.addLabel(12, y, 100, 12, 0xFFF0F0F0, str);
            str = StringUtils.translate("simplematica.gui.label.schematic_verifier.status.done_correct_total", c, t);
    ...
    private int createButton(int x, int y, int width, ButtonListener.Type type)
    {
        ...
        switch (type)
        {
            case SET_RESULT_MODE_ALL:
                ...
                {
                    label = StringUtils.translate("simplematica.gui.label.schematic_verifier_display_type.all_not_ignored");
                }
            ...
            case START:
                ...
                {
                    label = StringUtils.translate("simplematica.gui.button.schematic_verifier.resume");
                }
                else
                {
                    label = StringUtils.translate("simplematica.gui.button.schematic_verifier.start");
                    enabled = this.verifier.isActive() == false;
                }
                break;

            case STOP:
                label = StringUtils.translate("simplematica.gui.button.schematic_verifier.stop");
                enabled = this.verifier.isActive();
                break;

            case RESET_VERIFIER:
            {
                label = StringUtils.translate("simplematica.gui.button.schematic_verifier.reset_verifier");
            ...
            case SET_LIST_TYPE:
            {
                ...
                label = StringUtils.translate("simplematica.gui.button.schematic_verifier.range_type", str);
                break;
            }

            case RESET_IGNORED:
                label = StringUtils.translate("simplematica.gui.button.schematic_verifier.reset_ignored");
            ...
            case TOGGLE_INFO_HUD:
            {
                ...
                String str = (val ? TXT_GREEN : TXT_RED) + StringUtils.translate("simplematica.message.value." + (val ? "on" : "off")) + TXT_RST;
                label = StringUtils.translate("simplematica.gui.button.schematic_verifier.toggle_info_hud", str);
                break;
            }
    ...
    public void onSelectionChange(@Nullable BlockMismatchEntry entry)
    {
        if (entry != null)
        {
            ...
            if (Configs.InfoOverlays.VERIFIER_OVERLAY_ENABLED.getBooleanValue() == false)
            {
                ...
                InfoUtils.showGuiOrInGameMessage(MessageType.WARNING, "simplematica.message.warn.schematic_verifier.overlay_disabled", name, hotkeyName, hotkeyVal);
            }
        }
    }
        ...
        public void actionPerformedWithButton(ButtonBase button, int mouseButton)
        {
            switch (this.type)
            {
                ...
                case START:
                    ...
                    {
                        this.parent.addMessage(MessageType.ERROR, "simplematica.error.generic.schematic_world_not_loaded");
                    }
