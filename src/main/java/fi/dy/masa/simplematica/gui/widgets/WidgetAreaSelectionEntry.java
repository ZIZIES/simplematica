...
public class WidgetAreaSelectionEntry extends WidgetDirectoryEntry
{
    ...
    public void postRenderHovered(GuiContext ctx, int mouseX, int mouseY, boolean selected)
    {
        ...
        if (selection != null)
        {
            ...
            if (o == null)
            {
                ...
                str = StringUtils.translate("simplematica.gui.label.origin.auto");
            }
            else
            {
                str = StringUtils.translate("simplematica.gui.label.origin.manual");
            }

            String strOrigin = String.format("x: %d, y: %d, z: %d (%s)", o.getX(), o.getY(), o.getZ(), str);
            text.add(StringUtils.translate("simplematica.gui.label.area_selection_origin", strOrigin));

            int count = selection.getAllSubRegionBoxes().size();
            text.add(StringUtils.translate("simplematica.gui.label.area_selection_box_count", count));
        }
			...
			implements IButtonActionListener
	{
		...
		public void actionPerformedWithButton(ButtonBase button, int mouseButton)
		{
			...
			{
				String title = "simplematica.gui.title.rename_area_selection";
			...
			else if (this.type == ButtonType.COPY)
			{
				...
				if (selection != null)
				{
					String title = StringUtils.translate("simplematica.gui.title.copy_area_selection", selection.getName());
				...
				{
					this.widget.parent.getSelectionManagerGui().addMessage(MessageType.ERROR, "simplematica.error.area_selection.failed_to_load");
				}
			}
		...
		{
			RENAME("simplematica.gui.button.rename"),
			COPY("simplematica.gui.button.copy"),
			CONFIGURE("simplematica.gui.button.configure"),
