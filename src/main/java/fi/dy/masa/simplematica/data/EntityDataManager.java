...
import fi.dy.masa.simplematica.scheduler.info_hud.InfoHudSync;
...
//                    return;
    ...
    public void reset(boolean isLogout)
    {
        if (isLogout)
        {
            Simplematica.debugLog("EntityDataManager#reset() - log-out");
            HANDLER.reset(this.getNetworkChannel());
        ...
        {
            Simplematica.debugLog("EntityDataManager#reset() - dimension change or log-in");
...
//        return MathUtils.min(result, clamp);
    ...
    public void setServuxVersion(String ver)
    {
        if (ver != null && ver.isEmpty() == false)
        {
            ...
            Simplematica.debugLog("LitematicDataChannel: joining Servux version {}", ver);
        }
        else
        {
    ...
    public boolean receiveServuxMetadata(CompoundData data)
    {
        if (!DataManager.getInstance().hasIntegratedServer())
        {
            Simplematica.debugLog("LitematicDataChannel: received METADATA from Servux");
            ...
            if (Configs.Generic.ENTITY_DATA_SYNC.getBooleanValue())
            {
                ...
                {
                    Simplematica.LOGGER.warn("LitematicDataChannel: Mis-matched protocol version! (Expected: {} but got {} running on: {})", ServuxLitematicaPacket.PROTOCOL_VERSION, version, servux);
                ...
                Simplematica.debugLog("LitematicDataChannel: Connected to: {}", servux);
...
//    }
    ...
    public void requestServuxBulkEntityData(ChunkPos chunkPos, int minY, int maxY)
    {
        ...
        Simplematica.debugLog("EntityDataManager#requestServuxBulkEntityData(): for chunkPos {} to Servux (minY [{}], maxY [{}])", chunkPos.toString(), minY, maxY);
    ...
    public void requestBackupBulkEntityData(ChunkPos chunkPos, int minY, int maxY)
    {
        ...
        Simplematica.debugLog("EntityDataManager#requestBackupBulkEntityData(): for chunkPos {} (minY [{}], maxY [{}]) // Request --> TE: [{}], E: [{}]", chunkPos.toString(), minY, maxY, teSet.size(), entList.size());
    ...
    private boolean markBackupBlockEntityComplete(ChunkPos chunkPos, BlockPos pos)
    {
        ...
        {
            Simplematica.LOGGER.warn("EntityDataManager#markBackupBlockEntityComplete() - Marking ChunkPos {} - Block Entity at [{}] as complete.", chunkPos.toString(), pos.toShortString());
        }
        ...
        if (this.pendingChunks.contains(chunkPos))
        {
            if (this.pendingBackupChunk_BlockEntities.containsKey(chunkPos))
            {
                ...
                if (teSet.contains(pos))
                {
                    ...
                    if (teSet.isEmpty())
                    {
                        Simplematica.debugLog("EntityDataManager#markBackupBlockEntityComplete(): ChunkPos {} - Block Entity List Complete!", chunkPos.toString());
    ...
    private boolean markBackupEntityComplete(ChunkPos chunkPos, int entityId)
    {
        ...
        if (Reference.DEBUG_MODE)
        {
            Simplematica.LOGGER.warn("EntityDataManager#markBackupEntityComplete() - Marking ChunkPos {} - EntityId [{}] as complete.", chunkPos.toString(), entityId);
        }
        ...
        if (this.pendingChunks.contains(chunkPos))
        {
            if (this.pendingBackupChunk_Entities.containsKey(chunkPos))
            {
                ...
                if (entSet.contains(entityId))
                {
                    ...
                    if (entSet.isEmpty())
                    {
                        Simplematica.debugLog("EntityDataManager#markBackupEntityComplete(): ChunkPos {} - EntitiyList Complete!", chunkPos.toString());
...
//                Vec3 pos = DataTypeUtils.readVec3dFromListTag(ent);
            ...
            Simplematica.debugLog("EntityDataManager#handleBulkEntityData(): chunkPos {} received TE: [{}], and E: [{}] entiries from Servux", chunkPos.toString(), tileList.size(), entityList.size());
        }
    }
    ...
    private void checkForPendingChunkTimeout(ChunkPos pos)
    {
            ...
            (this.getIfReceivedBackupPackets() && this.hasPendingChunk(pos)))
        {
            ...
            {
                Simplematica.debugLog("EntityDataManager#checkForPendingChunkTimeout(): chunkPos {} has timed out waiting for data, marking complete without Receiving Entity Data.", pos.toString());
