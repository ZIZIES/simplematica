...
//                }
                ...
                case PACKET_S2C_NBT_RESPONSE_DATA ->
                {
                    ...
                    Simplematica.debugLog("ServuxLitematicaHandler#decodeClientData(): received Litematic Data Packet Slice of size {} (in bytes) // reading session key [{}]", packet.getTotalSize(), this.readingSessionKey);
                    ...
                    if (fullPacket != null)
                    {
                        ...
                        {
                            Simplematica.LOGGER.error("ServuxLitematicaHandler#decodeClientData(): Entity Data: error reading fullBuffer [{}]", e.getLocalizedMessage());
                        }
                    }
                }
                default ->
                        Simplematica.LOGGER.warn("ServuxLitematicaHandler#decodeClientData(): received unhandled packetType {} of size {} bytes.", packet.getPacketType(), packet.getTotalSize());
            }
        }
    }
    ...
    private void handleBulkData(@Nullable CompoundData data, final int packetSize)
    {
        ...
        Simplematica.debugLog("handleBulkData: received task: {} [Bytes: {} / {}]", task, packetSize, data.sizeInBytes());
...
//                {
//                    Simplematica.LOGGER.info("handleBulkData(): Received litematic '{}' from the server", schemPair.getLeft().getFile().toAbsolutePath().toString());
//
...
//                    buffer.writeNbt(packet.getCompound());
                    ...
                    {
//                            Simplematica.LOGGER.warn("[Servux Paste]: Slicing Oversided Schematic for Servux Paste ...");
//                            this.sliceForServux(schematicPlacement.getSchematic(), nbt, maxSize, printMessage);
                        InfoUtils.showGuiOrInGameMessage(Message.MessageType.ERROR, "simplematica.message.error.placement_paste_too_large_for_servux");
                    }
                    else
                    {
                ...
                {
                    Simplematica.LOGGER.error("ServuxLitematicaHandler#encodeServerData(): Exception encoding packet for PacketSplitter; {}", e.getLocalizedMessage());
                }
            }
    ...
    public void tickFailures()
    {
        ...
        {
            Simplematica.LOGGER.warn("ServuxLitematicaHandler$tickFailures(): encountered [{}] sendPayload failures, cancelling any Servux join attempt(s)", this.maxFailures());
            this.servuxRegistered = false;
