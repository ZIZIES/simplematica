...
public class TemporaryWorldManager implements AutoCloseable
{
	...
	protected synchronized TemporaryWorldHolder getTemporaryWorld(String worldName)
	{
		...
		if (!this.tempWorlds.containsKey(worldName))
		{
			...
			Simplematica.debugLog("TemporaryWorldManager: Created New temporary world: '{}' (No origin / size provided)", worldName);
		}
	...
	public synchronized TemporaryWorldHolder getTemporaryWorld(String worldName, BlockPos origin, Vec3i size)
	{
		...
		if (!this.tempWorlds.containsKey(worldName))
		{
			...
			Simplematica.debugLog("TemporaryWorldManager: Created New temporary world: '{}', at '{}' with a size of: '{}'", worldName, origin.toShortString(), size.toShortString());
		}
	...
	public synchronized void removeTemporaryWorld(String worldName)
	{
		...
		try (TemporaryWorldHolder removed = this.tempWorlds.remove(worldName))
		{
			if (removed != null)
			{
				Simplematica.debugLog("TemporaryWorldManager: Removed temporary world: '{}'", worldName);
			}
		}
	...
	public synchronized void setOriginAndSize(String worldName, BlockPos origin, Vec3i size)
	{
		...
		if (!this.tempWorlds.containsKey(worldName))
		{
			...
			this.tempWorlds.put(worldName, new TemporaryWorldHolder(origin, size));
			Simplematica.debugLog("TemporaryWorldManager: Replaced temporary world: '{}', at '{}' with a size of: '{}'", worldName, origin.toShortString(), size.toShortString());
		}
	}
	...
	public synchronized List<ChunkPos> getChunks(String worldName)
	{
		...
		if (this.tempWorlds.containsKey(worldName))
		{
			...
			if (world.isEmpty())
			{
				Simplematica.LOGGER.error("TemporaryWorldManager#getChunks(): Temporary world: '{}' is empty!  Cannot replace a chunk that was not calculated!", worldName);
			}
			else
			{
		...
		{
			Simplematica.LOGGER.error("TemporaryWorldManager#getChunks(): Temporary world: '{}'; was not found!", worldName);
		}
...
//			if (world.isEmpty())
//			{
//				Simplematica.LOGGER.error("TemporaryWorldManager#replaceChunkAt(): Temporary world: '{}' is empty!  Cannot replace a chunk that was not calculated!", worldName);
//				return false;
//			}
//
//			if (!world.chunkList().contains(Pair.of(cx, cz)))
//			{
//				Simplematica.LOGGER.error("TemporaryWorldManager#replaceChunkAt(): Temporary world: '{}' is empty!  Cannot replace chunk [cx: {}, cz: {}] outside of the requested Area Size", worldName, cx, cz);
//				return false;
//			}
//
//			// TODO --> create a task
...
//			{
//				Simplematica.debugLog("TemporaryWorldManager#replaceChunkAt(): Temporary world: '{}' --> Replaced Chunk at [cx: {}, cz: {}] successfully!", worldName, cx, cz);
//				return true;
//			}
//			else
//			{
...
//		}
//
//		Simplematica.LOGGER.error("TemporaryWorldManager#replaceChunkAt(): Temporary world: '{}'; was not found!", worldName);
//		return false;
//	}
