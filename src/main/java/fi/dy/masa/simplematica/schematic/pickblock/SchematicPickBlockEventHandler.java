package fi.dy.masa.simplematica.schematic.pickblock;
...
import fi.dy.masa.simplematica.interfaces.ISchematicPickBlockEventListener;
import fi.dy.masa.simplematica.interfaces.ISchematicPickBlockEventManager;
import fi.dy.masa.simplematica.interfaces.ISchematicPickBlockSlotHandler;
...
public class SchematicPickBlockEventHandler implements ISchematicPickBlockEventManager
{
	...
	public boolean executePickBlockHandler(Level world, BlockPos pos, ItemStack stack)
	{
		if (this.slotHandler != null)
		{
			...
			{
				Simplematica.LOGGER.warn("SchematicPickBlockEventHandler: Processing cancelled by: {} during 'executePickBlockHandler'", this.slotHandler.getName().get());
			...
			{
				Simplematica.LOGGER.error("SchematicPickBlockEventHandler: Error processing 'executePickBlockHandler' from: {}", this.slotHandler.getName().get());
				this.resetCancelled();
			}
			else
			{
	...
	public boolean onSchematicPickBlockStart(boolean closest)
	{
		for (ISchematicPickBlockEventListener handler : this.handlers)
		{
			...
			else
			{
				...
				if (result == SchematicPickBlockEventResult.CANCEL)
				{
					Simplematica.LOGGER.warn("SchematicPickBlockEventHandler: Processing cancelled by: {} during 'onSchematicPickBlockStart'", handler.getName().get());
					this.processingCancelled = true;
					this.processingCancelledBy = handler.getName();
				}
				else if (result == SchematicPickBlockEventResult.ERROR)
				{
					Simplematica.LOGGER.error("SchematicPickBlockEventHandler: Error processing 'onSchematicPickBlockStart' from: {}", handler.getName().get());
					this.resetCancelled();
				}
				else
				{
	...
	public boolean onSchematicPickBlockPreGather(Level schematicWorld, BlockPos pos, BlockState expectedState)
	{
		for (ISchematicPickBlockEventListener handler : this.handlers)
		{
			...
			else
			{
				...
				if (result == SchematicPickBlockEventResult.CANCEL)
				{
					Simplematica.LOGGER.warn("SchematicPickBlockEventHandler: Processing cancelled by: {} during 'onSchematicPickBlockPreGather'", handler.getName().get());
					this.processingCancelled = true;
					this.processingCancelledBy = handler.getName();
				}
				else if (result == SchematicPickBlockEventResult.ERROR)
				{
					Simplematica.LOGGER.error("SchematicPickBlockEventHandler: Error processing 'onSchematicPickBlockPreGather' from: {}", handler.getName().get());
					this.resetCancelled();
				}
				else
				{
	...
	public boolean onSchematicPickBlockPrePick(Level schematicWorld, BlockPos pos, BlockState expectedState, ItemStack stack)
	{
		for (ISchematicPickBlockEventListener handler : this.handlers)
		{
			...
			else
			{
				...
				if (result == SchematicPickBlockEventResult.CANCEL)
				{
					Simplematica.LOGGER.warn("SchematicPickBlockEventHandler: Processing cancelled by: {} during 'onSchematicPickBlockPrePick'", handler.getName().get());
					this.processingCancelled = true;
					this.processingCancelledBy = handler.getName();
				}
				else if (result == SchematicPickBlockEventResult.ERROR)
				{
					Simplematica.LOGGER.error("SchematicPickBlockEventHandler: Error processing 'onSchematicPickBlockPrePick' from: {}", handler.getName().get());
					this.resetCancelled();
				}
				else
				{
