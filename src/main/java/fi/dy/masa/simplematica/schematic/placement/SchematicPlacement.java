    ...
    */
    ...
    public ImmutableMap<String, Box> getSubRegionBoxes(RequiredEnabled required)
    {
        ...
        for (Map.Entry<String, SubRegionPlacement> entry : this.relativeSubRegionPlacements.entrySet())
        {
            ...
            if (areaSize == null)
            {
                Simplematica.LOGGER.warn("SchematicPlacement.getSubRegionBoxes(): Size for sub-region '{}' not found in the schematic '{}'", name, this.schematic.getMetadata().getName());
                continue;
            }
    ...
    public ImmutableMap<String, Box> getSubRegionBoxFor(String regionName, RequiredEnabled required)
    {
        ...
        if (placement != null)
        {
            if (placement.matchesRequirement(required))
            {
                ...
                {
                    Simplematica.LOGGER.warn("SchematicPlacement.getSubRegionBoxFor(): Size for sub-region '{}' not found in the schematic '{}'", regionName, this.schematic.getMetadata().getName());
                }
            }
        }
    ...
    public void moveSubRegionTo(String regionName, BlockPos newPos, IStringConsumer feedback)
    {
        if (this.isLocked())
        {
            feedback.setString("simplematica.message.placement.cant_modify_is_locked");
            return;
        }
    ...
    public void setSubRegionRotation(String regionName, Rotation rotation, IMessageConsumer feedback)
    {
        if (this.isLocked())
        {
            feedback.addMessage(MessageType.ERROR, "simplematica.message.placement.cant_modify_is_locked");
            return;
        }
    ...
    public static SchematicPlacement fromJson(JsonObject obj)
    {
            ...
            JsonUtils.hasArray(obj, "placements"))
        {
            ...
            if (schematic == null)
            {
                Simplematica.LOGGER.warn("Failed to load schematic '{}'", file.toAbsolutePath());
                return null;
            }
            ...
            if (posArr.size() != 3)
            {
                Simplematica.LOGGER.warn("Failed to load schematic placement for '{}', invalid origin position", file.toAbsolutePath());
                return null;
            }
