...
//	private final LinkedBlockingQueue<PlacementManagerTask> queueUnload = new LinkedBlockingQueue<>();
	...
	private static int calculateMaxThreads()
	{
		...
		Simplematica.LOGGER.info("Placement Manager calculated thread limit: [{}]", String.format("%02d/%02d", MIN_PLATFORM_THREADS, result));
		return result;
	}
...
//				Simplematica.LOGGER.error("CPU Count: {} / Safe Count: {}", Runtime.getRuntime().availableProcessors(), this.calculateDefaultSafeThreadCount());
				Simplematica.LOGGER.info("Resetting Placement Manager Thread count from config change [{} -> {}]", lastCount, count);
	...
	{
//		Simplematica.LOGGER.error("checkThreadCount(): count: {} // SAFE: {} // configured: {}", this.threadCount, this.calculateDefaultSafeThreadCount(), this.getConfiguredThreadCount());
	...
	public void start()
	{
		...
		Simplematica.LOGGER.info("Starting [{}] Placement Manager Daemon threads", this.threadMap.size());
	...
	public void stop()
	{
		Simplematica.LOGGER.info("Stopping [{}] Placement Manager Daemon threads", this.threadMap.size());
		Set<String> keys = this.threadMap.keySet();
		...
		for (String key : keys)
		{
			...
			{
				// Busy
				Simplematica.LOGGER.warn("Thread [{}] is currently busy, and shouldn't be stopped", key);
			}
			catch (IllegalStateException is)
			{
				// Terminated already
...
//			case PlacementManagerTaskUnload tU -> this.queueUnload.offer(newTask);
		...
		if (empty)
		{
			...
			{
				Simplematica.LOGGER.error("addTask: [EMPTY] Waking up threads...");
			}
	...
	public void onClientTick(Minecraft mc)
	{
		...
		if ((now - this.lastTick) > this.getTaskInterval())
		{
			if (mc.level != null)
			{
				...
				{
					Simplematica.debugLog("PlacementManagerDaemonHandler:  All tasks complete");
//					DataManager.getSchematicPlacementManager().setVisibleSubChunksNeedsUpdate();
	...
	private void ensureThreadsAreAlive()
	{
		...
		if (count > 0)
		{
			...
			Simplematica.debugLog("PlacementManagerDaemonHandler: {} tasks detected --> checking Thread states", count);
