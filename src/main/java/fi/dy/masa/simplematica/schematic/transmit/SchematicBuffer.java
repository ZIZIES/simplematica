...
public class SchematicBuffer
{
    ...
    public Path writeFile(Path dir)
    {
        if (!this.isComplete())
        {
            Simplematica.LOGGER.error("SchematicBuffer#writeFile(): Attempted to write incomplete buffer! Expected: {}, Received: {}", this.totalExpectedSlices, this.receivedSlices.get());
            return null;
        }
        ...
        if (!Files.isDirectory(dir))
        {
            ...
            {
                Simplematica.LOGGER.error("SchematicBuffer#writeFile(): Exception creating directory '{}'; {}", dir.toAbsolutePath().toString(), err.getLocalizedMessage());
                return null;
            }
        }
        ...
        if (Files.exists(file))
        {
            ...
            {
                Simplematica.LOGGER.error("SchematicBuffer#writeFile(): Exception deleting file '{}'; {}", file.toAbsolutePath().toString(), err.getLocalizedMessage());
                return null;
            }
        }
        ...
        {
            Simplematica.LOGGER.error("SchematicBuffer#writeFile(): Exception saving file '{}'; {}", file.toAbsolutePath().toString(), err.getLocalizedMessage());
            return null;
        }
        ...
        try
        {
            ...
            {
                Simplematica.LOGGER.error("SchematicBuffer#writeFile(): File size mismatch for '{}'! Expected: {} bytes, Actual: {} bytes. Deleting corrupted file.",
        ...
        {
            Simplematica.LOGGER.error("SchematicBuffer#writeFile(): Exception verifying file size for '{}'; {}", file.toAbsolutePath().toString(), err.getLocalizedMessage());
            return null;
        }

        Simplematica.debugLog("SchematicBuffer#writeFile(): Saved file '{}' successfully", file.toAbsolutePath().toString());
        this.buffer = null;
        return file;
    }
