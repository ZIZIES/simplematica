...
//        beNbt.copyFrom(newNbt);
    ...
    private static String legacyTextDeserializer(MutableComponent oldText, @Nonnull RegistryAccess registry)
    {
        ...
        {
            Simplematica.LOGGER.error("legacyTextDeserializer: Failed to convert MutableText to JSON; (falling back to just 'getString'); {}", err.getLocalizedMessage());
    ...
    private static @Nullable MutableComponent legacyTextSerializer(String json, @Nonnull RegistryAccess registry)
    {
        ...
        {
            Simplematica.LOGGER.error("legacyTextSerializer: Failed to convert JSON to MutableText; {}", err.getLocalizedMessage());
            return null;
        }
    }
...
//                LOGGER.debug("processSkullProfile(): customName1 [{}], disp [{}] // name [{}]", customName1, disp != null ? disp.getString() : "<null>", name);
            ...
            {
                Simplematica.LOGGER.warn("processSkullProfile(): Exception deserializing CustomName1 for Head Name.");
                name = customName1;
            }
        }
...
//                LOGGER.debug("processSkullProfile(): customName2 [{}], disp[{}] // name [{}]", customName2, disp != null ? disp.getString() : "<null>", name);
            ...
            {
                Simplematica.LOGGER.warn("processSkullProfile(): Exception deserializing CustomName2 for Head Name.");
                name = customName2;
            }
        }
