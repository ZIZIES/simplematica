...
public class PlacementManagerDaemonExecutor implements IThreadDaemonExecutor<PlacementManagerTask>
{
	...
	public void start()
	{
		...
		if (!this.isRunning())
		{
			Simplematica.debugLog("Executor: Starting");
			if (this.isPaused())
			{
	...
	{
		Simplematica.debugLog("Executor: Interrupt Signal: {}",
	...
	public void pause()
	{
		Simplematica.debugLog("Executor: Pausing");
	...
	public void resume()
	{
		...
		if (this.isPaused())
		{
			Simplematica.debugLog("Executor: Paused; Resuming");
			this.paused.set(false);
		}
	...
	public void stop()
	{
		Simplematica.debugLog("Executor: Stopping");
		if (!this.isPaused())
		{
			this.paused.set(true);
		}
...
//		this.maxTicks = PlacementManagerDaemonHandler.INSTANCE.getProfile().maxTicks();
		...
		this.ticks = 0L;
		Simplematica.debugLog("Executor: Running: [{}/{}]", this.isRunning(), this.isPaused());
		...
		Simplematica.debugLog("Executor: Stopped: [{}/{}]", this.isRunning(), this.isPaused());
	}
	...
	public boolean loopSafe()
	{
		...
		{
			Simplematica.debugLog("PlacementManagerDaemonExecutor#loopSafe: Exception: {}", err.getLocalizedMessage());
		}
