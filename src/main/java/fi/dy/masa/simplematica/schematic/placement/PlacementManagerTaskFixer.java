...
public class PlacementManagerTaskFixer extends PlacementManagerTask
{
	...
	protected Runnable buildTask()
	{
		return () ->
		{
			...
			if (!loaded.isEmpty())
			{
				Simplematica.debugLog("SchematicPlacementManager//FIXER: checking [{}] loaded chunks", loaded.size());
				loaded.forEach(c ->
							   {
...
//													  .schedulePendingTaskForNextTick(new PlacementManagerTaskUnload(this.worldSupplier(), c.x, c.z));
			...
			{
				Simplematica.debugLog("SchematicPlacementManager//FIXER: checking [{}] unloaded chunks", notLoaded.size());
				notLoaded.forEach(c ->
								  {
