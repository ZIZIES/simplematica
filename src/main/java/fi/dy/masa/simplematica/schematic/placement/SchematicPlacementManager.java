package fi.dy.masa.simplematica.schematic.placement;
...
import fi.dy.masa.simplematica.util.PositionUtils.ChunkPosDistanceComparator;
import fi.dy.masa.simplematica.util.RayTraceUtils;
...
//                    int minZ = ChunkPos.getZ(posLong) << 4;
    ...
    {
        return this.touchedVolumesInChunk.getOrDefault(ChunkPos.pack(chunkX, chunkZ), new CopyOnWriteArrayList<>());
    }

    public List<PlacementPart> getAllPlacementsTouchingChunk(BlockPos pos)
    {
        return this.touchedVolumesInChunk.getOrDefault(ChunkPos.pack(pos.getX() >> 4, pos.getZ() >> 4), new CopyOnWriteArrayList<>());
    }
    ...
    public void addSchematicPlacement(SchematicPlacement placement, boolean printMessages)
    {
        if (this.schematicPlacements.contains(placement) == false)
        {
            ...
            if (printMessages)
            {
                InfoUtils.showGuiMessage(MessageType.SUCCESS, StringUtils.translate("simplematica.message.schematic_placement_created", placement.getName()));
                ...
                if (Configs.InfoOverlays.WARN_DISABLED_RENDERING.getBooleanValue())
                {
                    ...
                    {
                        InfoUtils.showGuiAndInGameMessage(MessageType.WARNING, "simplematica.message.warn.layer_mode_currently_at", mode.getDisplayName());
                    }
                    ...
                    if (Configs.Visuals.ENABLE_RENDERING.getBooleanValue() == false)
                    {
                        ...
                        InfoUtils.showGuiAndInGameMessage(MessageType.WARNING, 8000,
                                "simplematica.message.warn.main_rendering_disabled", configName, hotkeyName, hotkeyVal);
                    }
                    ...
                    if (Configs.Visuals.ENABLE_SCHEMATIC_RENDERING.getBooleanValue() == false)
                    {
                        ...
                        InfoUtils.showGuiAndInGameMessage(MessageType.WARNING, 8000,
                                "simplematica.message.warn.schematic_rendering_disabled", configName, hotkeyName, hotkeyVal);
                    }
                    ...
                    if (Configs.Visuals.ENABLE_SCHEMATIC_BLOCKS.getBooleanValue() == false)
                    {
                        ...
                        InfoUtils.showGuiAndInGameMessage(MessageType.WARNING, 8000,
                                "simplematica.message.warn.schematic_blocks_rendering_disabled", configName, hotkeyName, hotkeyVal);
                    }
                }
            }
        }
        ...
        {
            InfoUtils.showGuiAndInGameMessage(MessageType.ERROR, "simplematica.error.duplicate_schematic_placement");
        }
    }
...
//        System.out.printf("rebuilding %d chunks: %s\n", chunks.size(), chunks);
    ...
    public void setPositionOfCurrentSelectionTo(BlockPos pos, Minecraft mc)
    {
        ...
        if (schematicPlacement != null)
        {
            ...
            {
                InfoUtils.showGuiOrActionBarMessage(MessageType.ERROR, "simplematica.message.placement.cant_modify_is_locked");
                return;
            }
            ...
            if (movingBox)
            {
                ...
                InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS, "simplematica.message.placement.moved_subregion_to", posStr);
            }
            ...
            else
            {
                ...
                if (old.equals(schematicPlacement.getOrigin()) == false)
                {
                    ...
                    InfoUtils.showGuiOrActionBarMessage(MessageType.SUCCESS, "simplematica.message.placement.moved_placement_origin", posStrOld, posStrNew);
                }
            }
        }
    }
