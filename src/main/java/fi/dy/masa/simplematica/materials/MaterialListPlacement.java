...
import fi.dy.masa.simplematica.scheduler.tasks.TaskCountBlocksPlacement;
...
public class MaterialListPlacement extends MaterialListBase
{
    ...
    public String getTitle()
    {
        ...
        if (type.isPerLayer() && this.isPerLayerUsable())
        {
            ...
            String key = type == BlockInfoListType.CURRENT_LAYER
                    ? "simplematica.gui.title.material_list.placement.current_layer"
                    : "simplematica.gui.title.material_list.placement.layer_and_below";

            return StringUtils.translate(key, this.getName(), range.getMaxLayerBoundary());
        }

        return StringUtils.translate("simplematica.gui.title.material_list.placement", this.getName());
    }
    ...
    public void reCreateMaterialList()
    {
        ...
        InfoUtils.showGuiOrInGameMessage(MessageType.INFO, "simplematica.message.scheduled_task_added");
    }
    ...
    private void logLayerTally(int fromY)
    {
        ...
        for (int y = this.layerCounts.getMinY(); y <= this.layerCounts.getMaxY(); ++y)
        {
            ...
            if (layerTotal > 0)
            {
                Simplematica.LOGGER.info("material list: y={} -> {} block(s), {} distinct state(s)",
                                       y, layerTotal, layer.size());
            }
        }

        Simplematica.LOGGER.info("material list: layers y={}..{} ({} populated), showing y={}..{}",
        ...
        {
            Simplematica.LOGGER.error("material list: per-layer counts sum to {} but the schematic holds {} -- the Y mapping is wrong",
