...
import fi.dy.masa.simplematica.mixin.screen.IMixinAbstractContainerScreen;
...
public class MaterialListHudRenderer implements IInfoHudRenderer
{
    ...
    public int render(GuiGraphicsExtractor graphics, int xOffset, int yOffset, HudAlignment alignment)
    {
        ...
        String title = GuiBase.TXT_BOLD + StringUtils.translate("simplematica.gui.button.material_list") + GuiBase.TXT_RST;
    ...
    protected String getFormattedCountString(int count, int maxStackSize)
    {
        ...
        if (count > maxStackSize)
        {
            if (boxCount >= 1.0)
            {
                return String.format("%d (%.2f %s)", count, boxCount, StringUtils.translate("simplematica.gui.label.material_list.abbr.shulker_box"));
            }
            else if (remainder > 0)
            {
